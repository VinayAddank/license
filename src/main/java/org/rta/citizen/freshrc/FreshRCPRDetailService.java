...
@Service
public class FreshRCPRDetailService extends AbstractDetailsService {
	...
	public GeneralDetails getDetails(String aadharNumber, String uniqueKey) {
		...
		RegistrationServiceResponseModel<FinancerFreshContactDetailsModel> financerFreshContactDetails = getFinancierContactDetails(
				vehicleRcId);
		if (!ObjectsUtil.isNull(financerFreshContactDetails.getResponseBody())) {
			...
			FinancerFreshContactDetailsModel financerFreshContactDetailsModel = financerFreshContactDetails
					.getResponseBody();
		...
		return GeneralDetails.builder().add(customerDetailsRequestModel).add(getVehicleDetails(vehicleRcId))
				.addChallanDetails(getChallanList(applicationModel.getPrNumber()))
				.addCrimeDetails(getCrimeDetails(applicationModel.getPrNumber()))
