...
public class AppSearchRCServiceImpl extends AbstractAppSearchService implements AppSearchService {

	private static final Logger log = Logger.getLogger(AppSearchRCServiceImpl.class);
    ...
    public ResponseModel<ApplicationStatusModel> getApplicationStatus(String appNo, Long sessionId) throws UnauthorizedException, NotFoundException {
        ...
        mdl.setApplicationType(ServiceType.REGISTRATION_CANCELLATION.getLabel());
        mdl.setServiceCode(ServiceType.REGISTRATION_CANCELLATION.getCode());
        mdl.setSubmittedOn(appEntity.getCreatedOn());
        if(Status.getStatus(appEntity.getLoginHistory().getCompletionStatus()) == Status.PENDING){
            try{
        ...
        if(overAllStatus == Status.PENDING || overAllStatus == Status.FRESH){
             ...
             if (ObjectsUtil.isNull(financierStatus) && Status.getStatus(appEntity.getLoginHistory().getCompletionStatus()) == Status.PENDING) {
               //--- is financed ----------
