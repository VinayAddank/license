...
@Service
public class DrivingLiceneceChangeOfAddressAuthendication extends AuthenticationService {

	private static final Logger log = Logger.getLogger(DrivingLiceneceChangeOfAddressAuthendication.class);
	...
	public ResponseModel<TokenModel> authenticate(AuthenticationModel citizen, ServiceType serviceType,
		...
		List<String> checkList = new ArrayList<>(Arrays.asList(ServiceType.LL_FRESH.getCode(),
				...
				ServiceType.DL_ENDORSMENT.getCode(), ServiceType.DL_RENEWAL.getCode(), ServiceType.DL_RETEST.getCode(),
				...
				ServiceType.DL_DUPLICATE.getCode()));
		...
		for (UserSessionEntity active : activeSessions) {
			...
			if (!ObjectsUtil.isNullOrEmpty(activeSessions)
					&& ServiceType.DL_CHANGE_ADDRESS.getCode().equalsIgnoreCase(active.getServiceCode())) {
				...
				if (activitiList.contains(actRes.getActiveTasks().get(0).getTaskDefKey())) {
					...
					throw new ConflictException(serviceType + " Application is pending at RTA office level");
				}
			}
	...
	protected Boolean isValidDlNumber(AuthenticationModel citizen) {
		...
		if (!StringsUtil.isNullOrEmpty(dlNumber) && dlNumber.startsWith("AP")
			...
			throw new IllegalArgumentException("DL Number is Not Valid");
	}
