...
public class AppSearchLLFServiceImpl extends AbstractAppSearchService implements AppSearchService {

	private static final Logger log = Logger.getLogger(AppSearchLLFServiceImpl.class);
	...
	public ResponseModel<ApplicationStatusModel> getApplicationStatus(String appNo, Long sessionId)
		...
		mdl.setApplicationType(ServiceType.LL_FRESH.getLabel());
		mdl.setServiceCode(ServiceType.LL_FRESH.getCode());
		...
		for (ApplicationApprovalHistoryEntity history : histpryEntityList) {
			...
			if (UserType.valueOf(history.getRtaUserRole()) == UserType.ROLE_CCO) {
				...
				mdl.setCcoActionTime(history.getCreatedOn());
				...
				mdl.setMviActionTime(history.getCreatedOn());
				...
				mdl.setAoActionTime(history.getCreatedOn());
				...
				mdl.setRtoActionTime(history.getCreatedOn());
				mdl.setRtoRemark(history.getComments());
			} else if (UserType.valueOf(history.getRtaUserRole()) == UserType.ROLE_EXAMINER) {
		...
		if(overAllStatus == Status.PENDING || overAllStatus == Status.FRESH){
			...
			if (ObjectsUtil.isNull(mdl.getExamStatus())) {
				mdl.setExamStatus(Status.PENDING);
			}
			...
			if (ObjectsUtil.isNull(mdl.getAoStatus()) && ObjectsUtil.isNull(mdl.getRtoStatus())) {
				if (mdl.getExamStatus() == Status.REJECTED) {
					mdl.setAoStatus(Status.NA);
					mdl.setRtoStatus(Status.NA);
				} else {
