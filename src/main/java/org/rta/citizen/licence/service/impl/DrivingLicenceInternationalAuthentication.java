...
@Service
public class DrivingLicenceInternationalAuthentication extends AuthenticationService {

	private static final Logger log = Logger.getLogger(DrivingLicenceInternationalAuthentication.class);
	...
	public ResponseModel<TokenModel> authenticate(AuthenticationModel citizen, ServiceType serviceType,
		...
		List<String> checkList = new ArrayList<>(Arrays.asList(ServiceType.LL_FRESH.getCode(),
				...
				ServiceType.DL_EXPIRED.getCode(), ServiceType.DL_MILITRY.getCode(),
				...
				if (null != holder) {
					...
					try {
						if ("Y".equalsIgnoreCase(holderDetail.getIsAdharVerify())
								&& driversLicenceDetailslist.get(0).getDlNo().equalsIgnoreCase(citizen.getDlNumber())
							...
							if (driversLicenceDetailslist.get(0).getLicenceHolderId()
									.equals(holderDetail.getLicenceHolderId()))
								validate = true;
						}
					} catch (Exception e) {
