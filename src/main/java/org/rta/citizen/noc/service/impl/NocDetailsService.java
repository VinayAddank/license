...
@Service
public class NocDetailsService extends AbstractDetailsService {

	private static final Logger log = Logger.getLogger(NocDetailsService.class);
    ...
    public GeneralDetails getDetails(String aadharNumber, String uniqueKey) {
        ...
        return GeneralDetails.builder().add(getCustomerDetails(vehicleRcId)).add(getVehicleDetails(vehicleRcId))
                ...
                .add(getTaxDetails(applicationModel.getTrNumber())).add(getPermitDetails(vehicleRcId)).add(getFitnessDetails(vehicleRcId))
