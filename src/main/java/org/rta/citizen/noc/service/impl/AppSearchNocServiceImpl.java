...
public class AppSearchNocServiceImpl extends AbstractAppSearchService implements AppSearchService {
	...
	public ResponseModel<ApplicationStatusModel> getApplicationStatus(String appNo, Long sessionId) throws UnauthorizedException, NotFoundException {
		...
		if (appEntity.getServiceCode().equalsIgnoreCase(ServiceType.NOC_ISSUE.getCode())) {
			mdl.setApplicationType(ServiceType.NOC_ISSUE.getLabel());
			mdl.setServiceCode(ServiceType.NOC_ISSUE.getCode());
		} else {
			mdl.setApplicationType(ServiceType.NOC_CANCELLATION.getLabel());
			mdl.setServiceCode(ServiceType.NOC_CANCELLATION.getCode());
		}
