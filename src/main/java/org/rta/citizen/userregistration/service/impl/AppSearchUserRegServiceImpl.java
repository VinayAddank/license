...
import org.rta.citizen.vehiclereassignment.service.impl.AppSearchVRServiceImpl;
...
public class AppSearchUserRegServiceImpl extends AbstractAppSearchService implements AppSearchService{
    ...
    public ResponseModel<ApplicationStatusModel> getApplicationStatus(String appNo, Long sessionId) throws UnauthorizedException, NotFoundException {
        ...
        mdl.setApplicationType(serviceType.getLabel());
        mdl.setServiceCode(serviceType.getCode());
        ...
        if(overAllStatus == Status.PENDING || overAllStatus == Status.FRESH){
        	 if (ObjectsUtil.isNull(mdl.getMviStatus()) && ((serviceType == ServiceType.PUC_USER_SIGNUP || serviceType == ServiceType.DEALER_SIGNUP
                     || serviceType == ServiceType.BODYBUILDER_SIGNUP || serviceType == ServiceType.DRIVING_INSTITUTE))) {
